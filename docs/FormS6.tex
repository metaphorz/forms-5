...
parameters). Beneath the input-vector slider, a \textbf{Mean} toggle recolours
the grid by the per-vertex wind averaged over all 100 input vectors (for the
current model, category, and land effect), and a \textbf{CSV} button exports the
six Form S-6 input variables (CP, $R_{\max}$, VT, WSP, CF, FFP) for all 100
vectors across the three categories. \textbf{Left-clicking} a vertex opens a floating windfield popup
